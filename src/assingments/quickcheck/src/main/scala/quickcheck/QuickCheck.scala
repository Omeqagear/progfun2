...
abstract class QuickCheckHeap extends Properties("Heap") with IntHeap {
  ...
  property("The smallest of 2 elements should be smallest in a previus empty Heap") = forAll{(n1: A, n2: A)=>
    val h = insert(n1, insert(n2, empty))
    findMin(h) == min(n1,n2)
  }

  property("Deleting minimal value in a Heap should result in an empty Heap") = forAll{(n: A)=>
    isEmpty(deleteMin(insert(n, empty )))
  }

  property("Recursivly finfing and deleting elements in a Heap should return same elements") = forAll{(h: H)=>
